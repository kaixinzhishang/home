...
public class YibogequWindow extends JFrame {
    ...
    public YibogequWindow(PlaylistManager playlistManager) {
        super("已播歌曲窗口");
        setSize(600, 400);
    ...
    private String getSongPathByName(String songName) {
        String query = "SELECT address FROM yibogequ WHERE name = ?";
    ...
    private void loadSongsFromDatabase() {
        ...
        String query = "SELECT name FROM yibogequ";
        try (Connection conn = DriverManager.getConnection(
                "jdbc:mysql://localhost:3306/ktvsjk?useUnicode=true&characterEncoding=utf8&useSSL=true", "root",
                "root");
                PreparedStatement pstmt = conn.prepareStatement(query);
    ...
    private void deleteSongFromDatabase(String songName) {
        String query = "DELETE FROM yibogequ WHERE name = ?";
        try (Connection conn = DriverManager.getConnection(
                "jdbc:mysql://localhost:3306/ktvsjk?useUnicode=true&characterEncoding=utf8&useSSL=true", "root",
                "root");
                PreparedStatement pstmt = conn.prepareStatement(query)) {
